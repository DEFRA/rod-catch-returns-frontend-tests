...
module.exports = function (action) {
  ...
  try {
    logger.debug('Calling waitForNav action ' + action)
    action()
    browser.waitUntil(function () {
      ...
      if (!hasChanged) {
        logger.info(`Waiting for page to load (loaded: ${hasChanged}).  [Old page id: ${util.inspect(oldPageId)}, current page id: ${util.inspect(currentPageId)}]`)
      }
      return hasChanged
  ...
  logger.info(`Page load complete.  [Old page: id=${util.inspect(oldPageId)}, url=${oldPageUrl}.  Current page: id=${util.inspect(currentPageId)}, url=${browser.getUrl()}]`)
}
