...
const waitForNav = require('../js/lib/wait-for-navigation-on-action')
...
*/
  ...
  checkErrorsOnPage (errorId, errorMessage) {
    const errRiverName = $(`a[href="${errorId}"]`).getText()
    expect(errRiverName).to.equal(errorMessage)
  }

  clickCancel () {
    console.log('About to click cancel link')
    const cancelBtn = $('#return-summary')
    waitForNav(function () {
      cancelBtn.click()
    })
  }
